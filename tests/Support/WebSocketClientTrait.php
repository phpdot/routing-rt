...
trait WebSocketClientTrait
{
     ...
     * @return resource
     */
    protected function openWebSocket(int $port, string $path = '/ws')
    {
     ...
     */
    protected function readHandshake($socket): string
    {
        $buffer = '';
    ...
    protected function encodeMaskedTextFrame(string $payload): string
    {
        return $this->encodeMaskedFrame(0x81, $payload); // FIN + text
    }

    protected function encodeMaskedBinaryFrame(string $payload): string
    {
        return $this->encodeMaskedFrame(0x82, $payload); // FIN + binary
    }

    protected function encodeMaskedFrame(int $firstByte, string $payload): string
    {
     ...
     * @param resource $socket
     */
    protected function readTextFrame($socket): string
    {
     ...
     */
    protected function readFrame($socket): array
    {
     ...
     * @param resource $socket
     */
    protected function readBytes($socket, int $count): string
    {
        $buffer = '';
